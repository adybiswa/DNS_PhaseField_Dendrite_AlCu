...
// Define constants for Al-5wt%Cu binary alloy
const double AL_CU_COMPOSITION = 0.05; // 5% Copper
const double SECONDARY_ARM_THRESHOLD = 20.0; // Threshold for secondary arm generation
const double PRIMARY_ARM_GROWTH_RATE = 0.5;
const double SECONDARY_ARM_GROWTH_RATE = 0.2;

// Function declarations
void GROW_SECONDARY_ARMS(double **phiphysical, double **ucphysical, int NP1_temp, int NP2_temp);
...
int main(int argc, char ** argv)
{
	...
	// Initial conditions/ field variables
	...
	for (int time_index = 1; time_index < NT; time_index++)
	{
		...
		// Call function to handle secondary arm growth
		GROW_SECONDARY_ARMS(phiphysical, ucphysical, NP1, NP2);
...
// Function to generate secondary arms based on conditions
void GROW_SECONDARY_ARMS(double **phiphysical, double **ucphysical, int NP1_temp, int NP2_temp)
{
	for (int i = 0; i < NP1_temp; ++i)
	{
		...
		{
			// Check if the primary trunk has formed sufficiently
			if (phiphysical[i][j] > SECONDARY_ARM_THRESHOLD)
			{
				// Logic for generating secondary arms
				for (int arm = 0; arm < 3; ++arm) // Example: Generate 3 secondary arms
				{
					int x_offset = (arm % 2 == 0) ? 1 : -1; // Alternating x direction
					int y_offset = (arm % 2 == 0) ? 1 : -1; // Alternating y direction
					if (i + x_offset < NP1_temp && j + y_offset < NP2_temp) {
						phiphysical[i + x_offset][j + y_offset] += SECONDARY_ARM_GROWTH_RATE; // Example modification for arm growth
					}
				}
			}
		}
